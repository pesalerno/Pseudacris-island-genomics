...
/home/salerno/Pseudacris/pyrad                        ## 1. Working directory                                 (all)
              ## 2. Loc. of non-demultiplexed files (if not line 18)  (s1)
              ## 3. Loc. of barcode file (if not line 18)             (s1)
vsearch                   ## 4. command (or path) to call vsearch (or usearch)    (s3,s6)
/opt/software/muscle/muscle3.8.31_i86linux32     ## 5. command (or path) to call muscle                  (s3,s7)
TGCA                     ## 6. Restriction overhang (e.g., C|TGCAG -> TGCAG)     (s1,s2)
2                         ## 7. N processors (parallel)                           (all)
10                         ## 8. Mindepth: min coverage for a cluster              (s4,s5)
...
@./edit/                       ## 18.opt.: loc. of de-multiplexed data                      (s2)
...
15                       ## 26.opt.: maxSNPs: (def=100). Paired (def=100,100)         (s7)
3,10                       ## 27.opt.: maxIndels: within-clust,across-clust (def. 3,99) (s3,s7)
